...
object AstVisitor : LatteBaseVisitor<AstNode>() {
    ...
    override fun visitDecl(ctx: LatteParser.DeclContext) =
        DeclStmt(ctx.type_().visit(), ctx.item().map { visitItem(it) })

    override fun visitCond(ctx: LatteParser.CondContext) =
        CondStmt(ctx.expr().visit(), ctx.stmt().visit())

    override fun visitCondElse(ctx: LatteParser.CondElseContext) =
        CondElseStmt(ctx.expr().visit(), ctx.stmt(0).visit(), ctx.stmt(1).visit())

    override fun visitWhile(ctx: LatteParser.WhileContext) =
        WhileStmt(ctx.expr().visit(), ctx.stmt().visit())
    ...
    private fun TerminalNode.visit() = SpannedText(text, symbol.line, symbol.startIndex, symbol.stopIndex)
}